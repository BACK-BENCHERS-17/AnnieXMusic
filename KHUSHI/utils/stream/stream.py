...
async def _send_stream_msg(
    ...
    try:
        result = await send_msg_invert_preview(app, original_chat_id, invert_text, reply_markup)
        if result:
            return result
    except Exception as e:
        _log.warning(f"[notify] invert preview failed for chat={original_chat_id}: {e}")
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                _nsfw_p = img and vidid not in _wl_p and await asyncio.get_event_loop().run_in_executor(None, is_thumb_nsfw_local, img)
                if _nsfw_p:
        ...
        if await is_active_chat(chat_id):
            ...
            # ── NSFW thumbnail check (always on, respects whitelist) ─────
            _nsfw_yt = img and vidid not in _wl and await asyncio.get_event_loop().run_in_executor(None, is_thumb_nsfw_local, img)
            if _nsfw_yt:
                return await _stop_and_block(_, chat_id, original_chat_id, title, vidid, user_name, user_id)
        ...
        if await is_active_chat(chat_id):
            ...
            # ── NSFW thumbnail check (always on, respects whitelist) ─────
            _nsfw_live = img and vidid not in _wl and await asyncio.get_event_loop().run_in_executor(None, is_thumb_nsfw_local, img)
            if _nsfw_live:
                return await _stop_and_block(_, chat_id, original_chat_id, title, vidid, user_name, user_id)
