...
- **Session 11 — Per-call emoji fallback + YAML href escapes**:
  - **Root cause of /help, /song, /reco silent failures**: the autowrap layer attaches `MessageEntityCustomEmoji` entities to every outgoing message, but Telegram rejects **specific** messages with `ENTITY_TEXT_INVALID` / `DOCUMENT_INVALID` (e.g. when an entity ID isn't reachable for the recipient context, when entity ranges happen to overlap an HTML construct, when the bot is messaging a user that has restricted custom emojis, etc). Pre-fix, every fallback layer (`raw_send` → `edit_caption` → `edit_text` → `send_photo` → `send_message`) hit the same rejection because **all** of them carried the same custom-emoji entities — so users saw nothing at all. The first-attempt fix of globally disabling autowrap on non-premium bots was wrong (premium emojis DID work in many places before), so it was reverted. **Real fix**: monkey-patched `pyrogram.Client.invoke` in `KHUSHI/utils/ui.py` (`_install_invoke_emoji_fallback()`) to catch `ENTITY_TEXT_INVALID` / `DOCUMENT_INVALID` from Telegram, strip every `MessageEntityCustomEmoji` from the raw RPC request via `_strip_custom_emoji_entities()`, and retry **once** with plain text. Premium emojis render whenever Telegram accepts them; gracefully fall back to plain Unicode otherwise — zero silent failures from any plugin or any code path. Also kept `disable_emoji_autowrap()` available as a manual escape hatch.
  - **YAML `<a href={N}>` escape fix**: 32 unquoted href patterns across all 5 language YAMLs (`en/ar/hi/ru/tr` — `help_1`, `play_21`, `queue_3`, `stream_1`, `start_5`, `start_6`) were quoted to `<a href=\"{N}\">` (proper YAML double-quote escape so the YAML loader doesn't choke on the inner quotes). Same fix applied to 4 f-string sites in `KHUSHI/utils/decorators_annie/{play,admins,language}.py` — switched outer quotes to single so inner `"..."` is valid Python.
- **Session 10 — Bypass parser fix + ANNIE brand restored**:
  - **Premium emoji bypass on /play, /song, /reco, /help fixed**: `KHUSHI/utils/raw_send.py` (the `invert_media` helper used by every now-playing notification) called `html_mod.HTML(client).parse(text)` **directly**, bypassing the `Parser.parse` monkeypatch installed in Session 9 — so all messages flowing through this path lost their premium emojis. `install_emoji_autowrap()` in `KHUSHI/utils/ui.py` now patches **three** entry points: `Parser.parse`, `HTML.parse`, AND `Markdown.parse`. Dedup in `_augment_with_custom_emoji` prevents double-wrapping when both Parser.parse and the underlying HTML/Markdown parser run. Verified: `🎵` in a `<blockquote>` now produces `MessageEntityCustomEmoji 5463107823946717464` via `HTML(client).parse()` directly.
  - **/help button DOCUMENT_INVALID fixed**: `strings/langs/en.yml` `help_1` had an unquoted `<a href={0}>` — when formatted with `SUPPORT_CHAT`, the malformed HTML caused Telegram to reject the message with `DOCUMENT_INVALID` on every edit/send path. Quoted the href properly: `<a href=\"{0}\">`.
  - **`_try_send_photo` now logs failures**: previously silently swallowed exceptions in all 3 fallback layers, so when /help failed users saw nothing AND no log explained why. Each layer now emits a WARN/ERROR with the chat_id and exception.
  - **ANNIE brand row restored on /play and /autoplay**: `KHUSHI/utils/ui.py` `BRAND` reset from `""` to `"<blockquote>🎵 <b>ᴀɴɴɪᴇ</b></blockquote>\n"` (premium-emoji rendered via autowrap). `KHUSHI/plugins/play.py._send_stream_msg` prepends `_BRAND` to the now-playing caption above the invisible-link anchor. `KHUSHI/plugins/autoplay.py` prepends `_BRAND` to `_autoplay_text()` and to all six `/autoplay on|off` reply variants.
- **Global premium-emoji autowrap (Session 9)**: even after Session 8 only `/start` showed premium emojis — every other plugin (`reco`, `language`, `play` error blockquotes, `settings`, `help`, etc.) used **raw inline unicode glyphs** (`🔥`, `❌`, `🎵`, …) directly in f-strings, bypassing the `_E` map, AND Pyrogram's **Markdown parser silently strips** any `<tg-emoji>` tag that does survive — so even messages that DID use `_E` lost their custom-emoji entities whenever they went through the default Markdown parser. Rather than refactor every plugin to use `_E`, added two new pieces in `KHUSHI/utils/ui.py`:
  1. **Reverse map `_UNICODE_TO_ID`** built from `_P`, with both VS16 (`\ufe0f`) and non-VS16 variants of every glyph registered (so `⚡` and `⚡️` both resolve to the same premium ID). Result: 136 glyph variants → 66 IDs.
  2. **`install_emoji_autowrap()`** monkeypatches `pyrogram.parser.parser.Parser.parse` so AFTER the original parser has produced its `{message, entities}` dict, it scans `message` for any known unicode glyph and **appends a `MessageEntityCustomEmoji`** at the correct UTF-16 offset/length, while skipping ranges already covered by an existing CustomEmoji entity (so explicit `<tg-emoji>` tags aren't double-wrapped).
  Hooked at the very top of `KHUSHI/__init__.py` so it runs before any client uses Parser. Net effect: every outgoing message — every plugin, every parse_mode (Default / Markdown / HTML), even `send_photo(caption=...)` and `edit_message_text` — now gets premium custom emojis automatically with zero plugin edits. Verified: 10/10 test glyphs produce `MessageEntityCustomEmoji` entities under all three parse modes.
