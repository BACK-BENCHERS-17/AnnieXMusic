...
- **Session 11 — Premium-bot guard + YAML href escapes**:
  - **Root cause of /help, /song, /reco silent failures discovered**: the autowrap layer was attaching `MessageEntityCustomEmoji` entities to **every** outgoing message, but **non-premium bots cannot send custom emojis** — Telegram rejects every such message with `ENTITY_TEXT_INVALID` / `DOCUMENT_INVALID`. All 3 fallback layers in `_try_send_photo` failed for the same reason, leaving users with no reply at all. Added `disable_emoji_autowrap()` in `KHUSHI/utils/ui.py` (sets `_AUTOWRAP_ENABLED=False`, makes the patched parsers no-op) and call it from `KHUSHI/core/bot.py` after `get_me()` whenever `me.is_premium` is False, with a clear WARN log. Premium emojis will automatically light up the day the bot account is upgraded to Telegram Premium.
