...
async def groupdata_handler(client, message: Message):
    ...
    text = (
        ...
        f"<b>▰▰▰▰▰▰▰▰▰▰▰▰▰</b>\n"
        f"<b>➻ ᴅᴀᴛᴇ ✧</b> {now}\n"
        f"<b>➻ ᴍᴇᴍʙᴇʀs ✧</b> `{total_members:,}`\n"
        f"<b>➻ ᴀᴅᴍɪɴs ✧</b> `{stats['admins']:,}`\n"
        f"<b>➻ ʙᴏᴛs ✧</b> `{stats['bots']:,}`\n"
        f"<b>➻ ᴢᴏᴍʙɪᴇs ✧</b> `{stats['deleted']:,}`\n"
        f"<b>➻ ʙᴀɴɴᴇᴅ ✧</b> `{stats['banned']:,}`\n"
        f"<b>➻ ᴘʀᴇᴍɪᴜᴍ ✧</b> `{stats['premium']:,}`\n"
        f"<b>➻ ʀᴇsᴛʀɪᴄᴛᴇᴅ ✧</b> `{stats['restricted']:,}`\n"
        f"<b>➻ ғᴀᴋᴇ ✧</b> `{stats['fake']:,}`\n"
        f"<b>▰▰▰▰▰▰▰▰▰▰▰▰▰</b>\n"
        f"<b>➻ ᴛɪᴍᴇ ᴛᴀᴋᴇɴ ✧</b> `{took}s`\n"
        f"<b>❅─────✧❅✦❅✧─────❅</b>"
    )
