"""DM Song Downloader — /song <query or URL> → quality picker → file."""
...
from ANNIEMUSIC.core.dir import DOWNLOAD_DIR
from ANNIEMUSIC.utils.ytdl_smart import get_base_ytdlp_opts
...
_BRAND = (
    ...
    " <b>ᴀɴɴɪᴇ ᴅᴏᴡɴʟᴏᴀᴅᴇʀ</b>"
)

_EM = {
    "dl":    "<emoji id='5373123633359855106'>⬇️</emoji>",
    "audio": "<emoji id='5463107823946717464'>🎵</emoji>",
    "video": "<emoji id='5375464961822695044'>🎬</emoji>",
    "wait":  "<emoji id='5420323339007281165'>⏳</emoji>",
    "ok":    "<emoji id='5367867546025223117'>✅</emoji>",
    "err":   "<emoji id='5210952531676504517'>❌</emoji>",
...
async def _search_yt(query: str) -> Optional[dict]:
    """Return first YouTube result dict or None."""
    try:
...
def _dl_yt(vid_id: str, quality: str) -> Optional[str]:
    """Blocking yt-dlp download. Returns file path or None."""
...
async def song_cmd(client, message: Message):
    ...
    if not info or not info.get("vid_id"):
        return await searching.edit_text(
            f"<blockquote>{_BRAND}</blockquote>\n\n"
            f"<blockquote>{_EM['err']} ɴᴏ ʀᴇsᴜʟᴛs ғᴏᴜɴᴅ ғᴏʀ: <b>{query}</b></blockquote>"
        )
...
async def song_dl_cb(client, cb: CallbackQuery):
    ...
    if quality == "cancel":
        ...
        try:
            ...
            await cb.answer("ᴄᴀɴᴄᴇʟʟᴇᴅ ✓", show_alert=False)
        return

    if not info:
        return await cb.answer("⚠️ Session expired. Send /song again.", show_alert=True)

    await cb.answer(f"⬇️ Downloading {_QUALITY_LABELS.get(quality, quality)} …", show_alert=False)
    ...
    if not file_path or not os.path.exists(file_path):
        try:
            await cb.message.edit_text(
                f"<blockquote>{_BRAND}</blockquote>\n\n"
                f"<blockquote>{_EM['err']} ᴅᴏᴡɴʟᴏᴀᴅ ғᴀɪʟᴇᴅ. ᴛʀʏ ᴀɴᴏᴛʜᴇʀ ǫᴜᴀʟɪᴛʏ ᴏʀ <code>/song</code> ᴀɢᴀɪɴ.</blockquote>"
            )
        except Exception:
            pass
        return
    ...
    if file_size_mb > 48:
        try:
            await cb.message.edit_text(
                f"<blockquote>{_BRAND}</blockquote>\n\n"
                f"<blockquote>{_EM['err']} ғɪʟᴇ ᴛᴏᴏ ʟᴀʀɢᴇ ({file_size_mb:.1f} ᴍʙ).\n"
                f"ᴘʟᴇᴀsᴇ ᴛʀʏ ᴀ ʟᴏᴡᴇʀ ǫᴜᴀʟɪᴛʏ.</blockquote>"
            )
        except Exception:
            pass
        os.remove(file_path)
    ...
    try:
        ...
        try:
            await cb.message.edit_text(
                f"<blockquote>{_BRAND}</blockquote>\n\n"
                f"<blockquote>{_EM['err']} sᴇɴᴅ ғᴀɪʟᴇᴅ: {type(e).__name__}</blockquote>"
            )
        except Exception:
            pass
    finally:
